...
# Third-Party
#from fpdf import FPDF #its for pdf export but doesnt really want to stop with the warning
...
class catalog:
    ...
    def display(self) -> dict[uuid.UUID, dict[str, str | int | list[int]]]:
        ...
        #self.sort_servers() can do, but will slow the program if done every time we want to display self.sort_servers()
    ...
    def remove_server(self, target_id: uuid.UUID):
        """Removes a server from the catalog by its ID"""
        self.servers = [server for server in self.servers if server.id != target_id]  # Implement a better way to remove?
    ...
    def export(self, format: str = "json", filename: str = "catalog", expo_path: str = "./"):
        """Exports the catalog to a file or database (Not implemented yet)"""
        data = self.display()  # Get the data to export

        if format == "json":
            with open(expo_path + filename, "w") as f:
                json.dump(data, f, indent=2)
        elif format == "csv":               # made with help of AI
            with open(expo_path + filename, "w", newline="") as f:
                if not data:
                    return
